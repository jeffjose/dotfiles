...
| 2026-07-17 ~19:20 | **Replacement decision made: buy a new 850 W Gold ATX 3.1 unit (~$110–150), NOT a like-for-like PRIME 1200.** Amazon prices checked: PRIME 1200 Platinum $319 new / $200 used — rejected (oversized: system peaks ~600 W, NVIDIA's official 3090 sizing is 750 W total; used PSU = unknown-history lottery again). ATX 3.1 units are certified for 2× transient excursions, specifically solving the 3090-spike problem the 2017-era PRIME predates. Picks: Seasonic Focus GX-850 (~$120) or Corsair RM850x (~$150); 1000 W only if a 5090-class upgrade is planned. NOTE: new PSU must use ONLY its own modular cables — pinouts differ per brand/generation. Working PRIME 1200 resells $150–200 → RMA/fan-fix could offset replacement cost. **Test 5 expected timings (boot 18:18): crash ~19:40–21:40 if fan irrelevant; >90% of baseline boots dead by ~22:20; all 76 dead by ~00:30. Alive at midnight = beyond recent record; alive 06:30 = conclusive heat-soak confirmation.** |
| 2026-07-18 09:01 | **TEST 5 CONCLUSIVE: SURVIVED THE NIGHT — 14 h 43 m and counting.** Driver loaded, normal evening use, then idle overnight — the exact conditions that produced 2.41 h ± 0.96 across 76 boots. Passed the all-time driver-loaded record at 00:30, the driverless record at ~04:00, and the decisive 12 h line at 06:18. A single change (Hybrid Mode OFF → PSU fan always on) turned the least stable configuration into the most stable since December. **ROOT CAUSE CONFIRMED: Seasonic PRIME 1200W Platinum heat-soaks at idle with its semi-passive fan stopped, degrading over months, hard-resetting the platform (power-good drop) with zero log trace. Every software theory (driver 580, ASPM, _DSM/BIOS, GSP, C-states, kernels) is closed.** Workaround ACTIVE: keep Hybrid Mode OFF — do not press that button back in. Endgame: (1) buy new 850 W Gold ATX 3.1 (~$120–150, Focus GX-850 / RM850x), swap with its OWN cables only; (2) side-sticker photo → serial → goodwill RMA email to Seasonic; (3) working PRIME 1200 resells $150–200. BIOS flash: optional hygiene someday, never was the cause. |
| 2026-07-19 10:18 | **Status check: 40 h 0 m uptime and counting — zero crashes since Hybrid Mode OFF.** Boot of Jul 17 18:18 still running. Longest uptime since January (pre-crash era). Conditions were the exact crash recipe: driver 570.211.01 loaded, GPU at P8/~30 W, normal desktop + heavy use (llama-server ran, was OOM-killed Jul 18 — unrelated to PSU, plain RAM exhaustion). No kernel errors beyond mundane USB/Bluetooth noise. **Root-cause confirmation now 40 h strong (~16× the 2.41 h baseline mean). Fix is holding.** Endgame unchanged: order 850 W Gold ATX 3.1, ask seller for receipt, submit Seasonic ticket. |
| 2026-07-18 ~09:30 | **RMA research (Seasonic policy):** warranty IS transferable to second owners (post-2012 units, original labels intact) — but requires the ORIGINAL purchase receipt; serial + manufacture date alone are officially insufficient. **No online serial→warranty checker exists** — warranty runs 12 yr from original purchase date (on the receipt, not in the serial), so the only real check is a support ticket at seasonic.com/support stating "second owner, no receipt — is this serial eligible?" **Action items: (1) message the seller asking for the original PSU receipt (highest-value move — with it, RMA is clean); (2) get serial off side sticker → submit ticket regardless (goodwill coin-flip); (3) order the new 850 W Gold ATX 3.1 in parallel — don't block the fix on RMA.** Sources: seasonic.com/warranty-policy, seasonic.com/rma-policy, seasonic.com/product-registration-landing. |
| 2026-09-15 | **Uptime 10 days** (boot Sep 5), driver 570.211.01 loaded, Hybrid Mode still OFF — fix holding at ~100× the 2.41 h crash-era baseline. Separately: the obsolete 570 apt pin blocked a ComfyUI install (needs CUDA 13 / driver 580+), so the pin and the crash-era GRUB params (`pcie_aspm=off`, GSP off, DPM off) were removed and the driver upgraded to 610.57.04. **The driver was exonerated by Test 4; nothing here affects the PSU fix.** See [woodwind-nvidia-driver-upgrade.md](woodwind-nvidia-driver-upgrade.md). |
